...
class Mesh:
    ...
    def assemble(self):
        ...
        for element in self.elements:
            if element.in_domain:
                ...
                indices = [node.index-1 for node in element.nodes]  # -1 because Python indices start at 0
    ...
    def apply_boundary(self):
        ...
        for node in np.ravel(self.nodes):
            if node.is_domain:
                ...
                idx = node.index - 1  # -1 because Python indices start at 0
    ...
    def plot_grid(self):
        for triangle in self.elements:
            ...
            color = 'green' if triangle.sigma == 9e7 else 'mediumblue'
        ...
        for row in self.nodes:
            for node in row:
                if node.is_domain:
                    plt.plot(node.x, node.y, 'ko', markersize=8)  # Plot a small black dot
                    plt.text(node.x, node.y, str(node.index), fontsize=5, c='w', ha='center', va='center')
    ...
    def assign_solution(self, V):
        for node in np.ravel(self.nodes):
            if node.is_domain:
                ...
                node.V = V[node.index - 1]  # -1 because Python indices start at 0
    ...
    def calc_current_across_surface(self, surface_x):
        ...
        for element in mesh.elements:
            ...
            # Check if the element intersects the surface at x = surface_x
            x_coords = [node.x for node in element.nodes]
            if (np.min(x_coords) < surface_x < np.max(x_coords)) or np.isclose(surface_x, np.min(x_coords)) or np.isclose(surface_x, np.max(x_coords)):
                # Compute the element's contribution to the total current
                total_current += element.J_mag * element.area

        total_current *= self.depth
...
print(f"Resistência do bloco: {voltage_diff / i_m * 1e6:.4f} µΩ")
